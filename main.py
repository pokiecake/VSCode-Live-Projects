...
#Treasure
treasure_img = pygame.image.load("Assets/TreasureChest.png")
treasure_w = treasure_img.get_width()
treasure_h = treasure_img.get_height()
treasures = []

#TURKEY
TURKEY_img = pygame.image.load("Assets/turkey.png")
TURKEY_w = TURKEY_img.get_width()
TURKEY_h = TURKEY_img.get_height()
TURKEY_visible = False
...
def fire_apple(x, y, mousePos = False):
    ...
    if mousePos == False:
        ...
        a = find_angle((centerX, centerY), mousePos)
        angle = a[0]
        apple_changeX = math.cos(angle) * bulletSpeed * a[1]
        apple_changeY = math.sin(angle) * bulletSpeed * a[2]
...
while running:
    ...
    for treasure in treasures:
        if currentRoom == treasure.room:
            draw(treasure_img, treasure.x, treasure.y)
            treasure_dim = (treasure.x, treasure.y, treasure_w, treasure_h)
            player_dim = (playerX, playerY, playerW, playerH)
            if (check_collisions(treasure_dim, player_dim)):
                print("collided")
